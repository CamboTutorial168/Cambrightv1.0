...
function getEmpSupply(people,cklist){
	...
	for (var i = 0; i < people.length; i++) {
		person=searchObj(cklist,people[i]['student_id']);
		list+='<tr>';
		list+="<td style='cursor:pointer;' title='Click to reset' onclick='resetRad(\""+people[i]["student_id"]+"\")'>"+people[i]["id_card"]+"</td>";
		list+="<td>"+people[i]["en_name"]+"</td>";
		...
		for(var day=firstDate;day<=lastDate;day++){
			...
			if(indexDateName==0){
					...
					for(var k=0; k<person.length; k++){
						...
						if(myday[2]==day && myday[1]==(monthNames.indexOf(token[0])+1)){
							...
							if(person[k]['is_absent']=='g'){
								list+="<span>G</span><input type='radio' disabled checked name='"+k+""+people[i].student_id+"'><br>";
								list+="P<input type='radio' disabled checked name='"+k+""+people[i].student_id+"'>";
							}else if(person[k]['is_absent']=='p'){
								list+="<span>G</span><input type='radio' disabled name='"+k+""+people[i].student_id+"'><br>";
								list+="P<input type='radio' disabled checked name='"+k+""+people[i].student_id+"'>";
							}else{
				...
				for(var k=0; k<person.length; k++){
					...
					if(myday[2]==dayNow && myday[1]==(monthNames.indexOf(token[0])+1)){
						...
						if(person[k]['is_absent']=='p'){
							list+=" class='today'><span>G</span><input type='radio' disabled name='"+k+""+people[i].student_id+"'><br>";
							list+="P<input type='radio' checked disabled name='"+k+""+people[i].student_id+"'>";
						}else if(person[k]['is_absent']=='g'){
							list+=" class='today'><span>G</span><input type='radio' checked disabled name='"+k+""+people[i].student_id+"'><br>";
							list+="P<input type='radio' disabled name='"+k+""+people[i].student_id+"'>";
						
						}else{
							list+=" class='today'><span>G</span><input type='radio' disabled ><br>";
							list+="P<input type='radio' disabled >";
							//list+="value=\"{'date':'"+token[1]+'/'+(monthNames.indexOf(token[0])+1)+'/'+day+"','student_id':'"+people[i]['student_id']+"'}\" class='ck_"+day+"' onchange='checking("+day+")'></td>";
						}
						list+="</td>";
						$("#ck-att").attr("disabled",true);
						break ready;
				...
				list+="<td class='today'><span>G</span><input type='radio' name='"+people[i].student_id+"'";
				list+="value=\"{'date':'"+token[1]+'/'+(monthNames.indexOf(token[0])+1)+'/'+day+"','student_id':'"+people[i]['student_id']+"','is_absent':'g'}\" class='ck_"+day+"' onchange='checking("+day+")'><br>";
				list+="P<input type='radio' name='"+people[i].student_id+"'";
				list+="value=\"{'date':'"+token[1]+'/'+(monthNames.indexOf(token[0])+1)+'/'+day+"','student_id':'"+people[i]['student_id']+"','is_absent':'p'}\" class='ck_"+day+"' onchange='checking("+day+")'></td>";
			}
